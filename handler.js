      ...
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          try {
            ...
            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'مرحبا ياصديقي, @user!').replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*without description*') :
                              (chat.sBye || this.bye || conn.bye || 'وداعا ياصديقي, @user!')).replace('@user', '@' + user.split('@')[0]);
...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    group: '*[ ℹ️ ] هاذا الأمر يعمل في المجموعات فقط.*',
    private: '*[ ℹ️ ] هاذا الأمر خاص بصاحب البوت.*',
    admin: '*[ ℹ️ ] هاذا الأمر خاص بمشرفين المجمرعة.*',
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ ⚠ ] Warning*', body: 'JITOSA', thumbnail: imagen1, sourceUrl: 'https://instagram.com/ovmar_1'}}}}, aa);
