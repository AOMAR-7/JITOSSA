...
global.authFile = `JitossaSession`;
...
let prekey = []
let directorio = readdirSync("./JitossaSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./JitossaSession/${files}`)
})
} 
...
const directories = ['./JitossaSession/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./JitossaSession/" + "creds.json")
