...
let handler = async (m, {
    ...
    if (!m.quoted) return m.reply("*التكلم مع الذكاء الإصطناعي بواصة الأوديو فقط*\n قم باإرسال الأوديو الخاص بك ورد عليه بهاذا الأمر ${usedPrefix}${command}");
...
async function gptAudio(audioBuffer) {
    try {
        ...
        const response = await fetch('https://chatopenai.de/wp-admin/admin-ajax.php', {
            method: 'POST',
            body: data
        });
